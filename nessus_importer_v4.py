...
async def get_latest_task_for_host(token: str, cts: Const, project_id: str, ipv4: str, client: httpx.AsyncClient) -> tuple[str, str | None, dict[str, str]]:
    response = await client.get(f"{cts.SCANFACTORY_URL}/api/tasks/?project_id={project_id}&tool=infrascan&sort=-mdate&status=6&host={ipv4}&limit=1&token={token}")
    if response.status_code != 200:
...
async def get_and_push_report(creds: Credentials, cts: Const, token: str, project_id: str, task_id: str, reports: dict[str, str], client: httpx.AsyncClient) -> None:
    for type, accept in [("nessus", "application/xml"), ("csv", "text/csv")]:
        async with client.stream(
            method="GET",
            url=f"{cts.SCANFACTORY_URL}/api{reports[type]}?token={token}",
...
async def main():
    ...
    async with httpx.AsyncClient(follow_redirects=True, headers={"Accept": "application/json"}) as client:
        ...
        tasks_: dict[str, list[tuple[str, dict[str, str]]]] = {}
        for project_id, task_id, reports  in await asyncio.gather(*tasks):
            if task_id or reports:
                tasks_.setdefault(project_id, []).append((task_id, reports))
        logging.info(f"Received tasks (10 latest showed): {json.dumps({project: tasks[:10] for project, tasks in tasks_.items()})}")

    async with httpx.AsyncClient(follow_redirects=True) as client:
        tasks = []
        for project_id, task_data in tasks_.items():
            for task_id, reports in task_data:
                tasks.append(asyncio.create_task(get_and_push_report(creds, cts, token, project_id, task_id, reports, client)))
